...
for i in ins:
    ...
    if "x" in i:
        dx = int(i.split("x=")[1])
        #dx = min(dx, len(matrix) - 1 - dx)
        new_matrix = [[False for _ in range(dx)] for _ in range(len(matrix))]
        for x in range(len(new_matrix[0])):
            for y in range(len(new_matrix)):
                new_matrix[y][x] = matrix[y][x] or (len(matrix[0]) > dx + len(new_matrix[0]) - x and matrix[y][dx + len(new_matrix[0]) - x])
    if "y" in i:
        dy = int(i.split("y=")[1])
        #dy = min(dy, len(matrix) - 1 - dy)
        new_matrix = [[False for _ in range(len(matrix[0]))] for _ in range(dy)]
        for x in range(len(new_matrix[0])):
            for y in range(len(new_matrix)):
                new_matrix[y][x] = matrix[y][x] or (len(matrix) > dy + len(new_matrix) - y and matrix[dy + len(new_matrix) - y][x])
    matrix = new_matrix
